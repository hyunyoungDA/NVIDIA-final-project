...
from voice_ordering import start_complete_voice_ordering
...
def initialize_session_state():
    ...
    if 'voice_ordering_active' not in st.session_state:
        st.session_state.voice_ordering_active = False
    if 'voice_ordering_completed' not in st.session_state:
        st.session_state.voice_ordering_completed = False
...
def show_payment_view():
    ...
    # 음성 챗봇으로 이미 결제 방법이 선택된 경우
    if st.session_state.get('selected_payment_method'):
        st.info(f"🎤 음성 주문으로 **{st.session_state.selected_payment_method}** 결제가 선택되었습니다.")
        
        if st.button(f"✅ {st.session_state.selected_payment_method}로 결제하기", use_container_width=True, type="primary"):
            process_payment_from_voice(st.session_state.selected_payment_method)
        
        st.markdown("---")
        st.markdown("다른 결제 방법을 원하시면 아래에서 선택하세요:")
...
def process_payment_from_voice(method):
    """음성 챗봇에서 선택된 결제 방법 처리"""
    st.session_state.order_number = generate_order_number()
    st.session_state.current_view = "processing"
    st.rerun()
...
def perform_face_recognition():
    ...
    try:
        # 얼굴 인식 진행 중 표시
        with st.spinner("얼굴 인식 중입니다... 카메라를 확인해주세요."):
            age = get_predicted_age()
            
        if age:
            st.session_state.face_recognition_age = age
            st.session_state.elderly_mode = age >= 60
            if age >= 60:
            ...
            st.session_state.face_recognition_age = None
            return None
