...
def get_langchain_response(user_message, conversation_history):
    ...
    2. **메뉴 보기:** 고객이 특정 메뉴를 보길 원하면 JSON 형식으로만 응답하세요: {{"action": "show_menu", "category": "메뉴의 상위 카테고리(메인, 사이드, 음료, 디저트)"}} 
    ...
    4. **주문:** 고객이 메뉴를 주문하면 JSON 형식으로만 응답하세요: {{"action": "add_to_cart", "items": [{{"name": "메뉴명", "quantity": 수량, "item_id": "item_xxx"}}]}}
    5. **결제:** 고객이 주문 완료나 결제 의사를 표현하면 JSON 형식으로만 응답하세요: {{"action": "proceed_to_payment"}}
    6. **기타 질문:** 위 규칙에 해당하지 않는 일반 대화나 질문에는 일반 텍스트로 응답하세요.
    
    중요: JSON 응답을 할 때는 JSON만 반환하고, 추가 텍스트나 설명을 포함하지 마세요.
    """
...
def process_ai_response(ai_response):
    try:
        ...
        def extract_json_from_text(text):
            ...
            if matches:
                # 가장 긴 JSON 문자열을 반환 (가장 완전한 JSON일 가능성이 높음)
        ...
        if json_str:
            ...
            if response_json.get('action') == 'add_to_cart':
                ...
                # JSON이 처리되었으므로 사용자 친화적인 메시지만 반환
...
def display_payment_page():
    ...
    with col2:
        if st.button("💳 결제하기", type="primary", key="finalize_payment"):
            ...
            st.info("5초 후 자동으로 얼굴 인식 페이지로 돌아갑니다.")
            time.sleep(5)
