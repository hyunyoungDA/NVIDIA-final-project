...
# 직접 URL 설정 (환경 변수 의존성 제거)
TARGET_URL1 = "http://localhost:8502"  # 일반 키오스크
TARGET_URL2 = "http://localhost:8503"  # 음성 키오스크
...
def main():
    ...
    # 나이 인식 완료 후 결과 표시
        ...
        if age < 65:
            st.subheader("🎯 65세 미만")
            st.info(f"추정 나이: {age}세 -> 자동으로 일반 키오스크로 이동합니다.")
            st.info(f"이동할 URL: {TARGET_URL1}")
            ...
            st.subheader("🍽 65세 이상")
            st.info(f"추정 나이: {age}세 -> 자동으로 음성 키오스크로 이동합니다.")
            st.info(f"이동할 URL: {TARGET_URL2}")
